...
class TemplateCard(QFrame):
    ...
    def _on_save_template(self):
        ...
        try:
            ...
            if not (
                0 <= s_hsv_color <= 255
                and 0 <= s_hsv_diff <= 255
                and 0 <= v_hsv_color <= 255
                and 0 <= v_hsv_diff <= 255
            ):
    ...
    def _update_template_combo(self):
        ...
        if self.template_combo.currentIndex() >= 0 and hasattr(self, "all_templates"):
            ...
            current_template_id = (
                f"{current_template.frame_id}/{current_template.template_id}"
            )
        ...
        for template in self.all_templates:
            display_name = (
                f"{template.frame_id}/{template.template_id} - {template.template_name}"
            )
            ...
            if image_path.exists():
                ...
                if self.template_image is not None:
                    ...
                    self.status_label.setText(
                        f"状态: 已加载模板 {template.template_name}"
                    )
                    # 启用按钮
                    self._update_button_states(True)
                else:
    ...
    def _apply_hsv_crop(self):
        ...
        try:
            ...
            self.apply_template_button.clicked.connect(
                lambda: self._restore_original(crop=True)
            )
    ...
    def _apply_hsv_original(self):
        ...
        try:
            ...
            self.apply_original_button.clicked.connect(
                lambda: self._restore_original(crop=False)
            )
    ...
    def _on_image_clicked(self, event):
        ...
        if 0 <= img_x < img_width and 0 <= img_y < img_height:
            ...
            # 转换为HSV
            hsv = cv2.cvtColor(np.array([[bgr]], dtype=np.uint8), cv2.COLOR_BGR2HSV)[0][
                0
            ]
            ...
            if self.is_crop_mode:
                if not hasattr(self, "start_pos") or self.start_pos is None:
                    ...
                    # 更新预览框
                    if (
                        hasattr(self, "current_image")
                        and self.current_image is not None
                    ):
                        x1, y1 = self.start_pos
                        x2, y2 = self.end_pos
                        # 裁剪并保存到template_image
                        self.template_image = self.current_image[
    ...
    def _on_new_template(self):
        ...
        bytes_per_line = 3 * width
        q_img = QImage(
            default_img.data, width, height, bytes_per_line, QImage.Format_RGB888
        ).rgbSwapped()
        pixmap = QPixmap.fromImage(q_img)
        self.image_label.setPixmap(
            pixmap.scaled(
