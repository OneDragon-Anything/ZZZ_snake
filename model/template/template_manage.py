...
class TemplateManager:
    ...
    def _get_hsv_mask(self, hsv_img, hsv_ranges):
        # 预计算边界数组（避免重复创建）
        lower = np.array(
            [hsv_ranges["h_lower"], hsv_ranges["s_lower"], hsv_ranges["v_lower"]],
            dtype=np.uint8,
        )
        upper = np.array(
            [hsv_ranges["h_upper"], hsv_ranges["s_upper"], hsv_ranges["v_upper"]],
            dtype=np.uint8,
        )
    ...
    def get_template_analysis(self, template):
        ...
        if hasattr(template, "image_path") and template.image_path:
            # 读取并缓存图像
            if (
                hasattr(template, "cropped_image")
                and template.cropped_image is not None
            ):
            ...
            # 缓存HSV图像
            template._analysis_cache["hsv_img"] = cv2.cvtColor(
                template_img, cv2.COLOR_BGR2HSV
            )

            # 计算并缓存掩码 (修改后的调用方式)
            hsv_ranges = self._get_hsv_ranges(template)
            template._analysis_cache["mask"] = self._get_hsv_mask(
                template._analysis_cache["hsv_img"], hsv_ranges
            )
            ...
            if contours:
                ...
                template._analysis_cache["area"] = cv2.contourArea(
                    template._analysis_cache["contour"]
                )
                ...
                if perimeter > 0:
                    if template.shape_type == "circle":
                        template._analysis_cache["shape_metric"] = (
                            4
                            * np.pi
                            * template._analysis_cache["area"]
                            / (perimeter**2)
                        )
                    elif template.shape_type == "rectangle":
                        _, (w, h), _ = cv2.minAreaRect(
                            template._analysis_cache["contour"]
                        )
                        rect_area = w * h
                        if rect_area > 0:
                            template._analysis_cache["shape_metric"] = (
                                template._analysis_cache["area"] / rect_area
                            )

        return template._analysis_cache

    def find_objects_by_features(
        self, template, image=None, hsv_image=None, search_in_crop=False
    ):
        try:
