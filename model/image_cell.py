        ...
        def dropEvent(self, event):
            for url in event.mimeData().urls():
                ...
                if file_path.lower().endswith((".png", ".jpg", ".jpeg", ".bmp")):
                    ...
                    # 显示处理结果
                    self.setPixmap(
                        QPixmap(file_path).scaled(400, 400, Qt.KeepAspectRatio)
                    )
                    ...
                    print(
                        f"裁剪区域: {cell.bounds[0]+3},{cell.bounds[1]+3} - {cell.bounds[2]-3},{cell.bounds[3]-3}"
                    )
                    print(
                        f"H值数组长度: {len(cell.h_values) if cell.h_values is not None else 0}"
                    )
